...
class H5Dataset(Dataset):
    def __init__(self, file_path, attributes, transform=None):
...
class ImageDataModule(pl.LightningDataModule):
    def __init__(self, batch_size, data_path, attributes, transform=None):
        super().__init__()
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            full_dataset = H5Dataset(self.data_path, attributes=self.attributes, transform=self.transform)
